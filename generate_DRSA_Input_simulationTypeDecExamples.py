...
simulation_bestPicker_DM = dict([]) #contain dictionary of reference sets # ex.: simulation_winnerPicker_DM['sim_X'] 
...
for subgroup in risk_groups['C1_moodys']:
	weights_best[subgroup] = k1*mth.exp(-k2*counter) # decreasing weights
...
for subgroup in risk_groups['C1_s&p']:
	...
	weights_worst[subgroup] = k1*(mth.exp(k2*counter)-k2) # increasing weights
	counter += 1
subgroup_weights_C1['best'] = weights_best
...
for subgroup in risk_groups['C2_moodys']:
	...
	#weights_worst[subgroup] = k1*(mth.exp(k2*counter)-k2) # increasing weights (original for the three simulations)
	counter += 1
...
for subgroup in risk_groups['C2_s&p']:
	...
	#weights_worst[subgroup] = k1*(mth.exp(k2*counter)-k2) # increasing weights (original for the three simulations)
	counter += 1
subgroup_weights_C2['best'] = weights_best
...
for subgroup in risk_groups['C3_s&p']:
	...
	#weights_worst[subgroup] = k1*(mth.exp(k2*counter)-k2) # increasing weights (original for the three simulations)
	counter += 1
subgroup_weights_C3['best'] = weights_best
...
for i in range(0, len(critDB)): # start filling class populations
	if(not(np.isnan(critDB.iloc[i, len(critDB.columns)-1]))): #check if this country was classified
		...
		if(not(missingValue)): # if this country was evaluated in all criteria, include in the population
			...
			if(clss == '1'):
				weights_best = subgroup_weights_C1['best']
				weights_worst = subgroup_weights_C1['worst']
			if(clss == '2'):
				weights_best = subgroup_weights_C2['best']
				weights_worst = subgroup_weights_C2['worst']
			if(clss == '3'):
				weights_best = subgroup_weights_C3['best']
				weights_worst = subgroup_weights_C3['worst']
			# get weights for each DM type
			final_weight_best = []
			final_weight_worst = []
			# get moody's weight
			if(critDB.iloc[i, len(critDB.columns)-5] in weights_best.keys()): #check if NaN
				moodyclss = critDB.iloc[i, len(critDB.columns)-5]
				final_weight_best += [weights_best[moodyclss]]
				final_weight_worst += [weights_worst[moodyclss]]
			# get sp weight
			if(critDB.iloc[i, len(critDB.columns)-4] in weights_best.keys()): #check if NaN
				spclss = critDB.iloc[i, len(critDB.columns)-4]
				final_weight_best += [weights_best[spclss]]
				final_weight_worst += [weights_worst[spclss]]
			# finalweight = mean between weights
			final_weight_best = np.mean(final_weight_best)
			final_weight_worst = np.mean(final_weight_worst)
			# assign the respective subjective weights for this country
			population_weights[clss + "_best"] = population_weights[clss + "_best"] + [final_weight_best]
...
for s in range(nSims):
	# best
	c1_objs = np.random.choice(a = population['1'], size = nDecExamples, replace = False, p = population_weights['1_best']) #sample n decision examples for this group
	c2_objs = np.random.choice(a = population['2'], size = nDecExamples, replace = False, p = population_weights['2_best']) #sample n decision examples for this group
	c3_objs = np.random.choice(a = population['3'], size = nDecExamples, replace = False, p = population_weights['3_best']) #sample n decision examples for this group
	simulation_bestPicker_DM['sim_' + str(s)] = c1_objs.tolist() + c2_objs.tolist()  + c3_objs.tolist() 
	# worst
	...
	simulation_worstPicker_DM['sim_' + str(s)] = c1_objs.tolist() + c2_objs.tolist()  + c3_objs.tolist() 
...
for s in range(nSims):
	...
	best_base_dataSet = open("inputData/drsa/simulation_wrt_decisionExType/best/drsa_countryRisk_" + str(baseYear) + "_sim" + str(s + 1) + ".isf","w")
	worst_base_dataSet = open("inputData/drsa/simulation_wrt_decisionExType/worst/drsa_countryRisk_" + str(baseYear) + "_sim" + str(s + 1) + ".isf","w")
	best_test_dataSet = open("inputData/drsa/simulation_wrt_decisionExType/best/test/drsa_countryRisk_" + str(baseYear) + "_sim" + str(s + 1) + ".isf","w")
	worst_test_dataSet = open("inputData/drsa/simulation_wrt_decisionExType/worst/test/drsa_countryRisk_" + str(baseYear) + "_sim" + str(s + 1) + ".isf","w")
